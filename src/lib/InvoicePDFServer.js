// Server-safe version of InvoicePDF (no 'use client' directive)
// This is used by the API route for server-side PDF generation
...
export default function InvoicePDFServer({ invoice, items, client, profile, currency = 'INR', logoSrc = null }) {
  ...
  // If no logoSrc was passed, fallback to the url
  const finalLogoSrc = logoSrc || profile?.company_logo_url;
